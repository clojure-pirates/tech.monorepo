(ns tech.core.resource.gc
  ...
  (:import [java.lang.ref ReferenceQueue]
           [java.lang Thread]
           [tech.core.resource GCReference GCSoftReference]
...
(defn watch-reference-queue
  ...
  (try
    (println :tech.core.resource.gc "Reference thread starting")
    (loop [continue? @run-atom]
      (when continue?
        (let [next-ref (.remove reference-queue 100)]
          ...
          (recur @run-atom))))
    (catch Throwable e
      (println :tech.core.resource.gc "!!Error in reference queue!!:" e)))
  (println :tech.core.resource.gc "Reference queue exiting"))
...
(defn start-reference-thread
  ...
  (when-not @*reference-thread*
    (let [run-atom (atom true)
      ...
      (.setName thread "tech.core.resource.gc ref thread")
      (.start thread)
