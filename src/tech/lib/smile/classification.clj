...
(defn- train-online
  ...
  (let [;;Do basic NN shit to make it work.  Users don't need to specify the
    ...
    (->> coalesced-dataset
         (map #(.learn untrained ^doubles (::dataset/features %)
                       (int (dtype/get-value (::dataset/label %) 0))))
         dorun)
    (when (= (:name entry-metadata) :svm)
      (let [^SVM sort-of-trained untrained]
        (.trainPlattScaling sort-of-trained
                            (->> (map ::dataset/features coalesced-dataset)
                                 object-array)
                            ^ints
                            (->> (map (comp int #(dtype/get-value % 0) ::dataset/label) coalesced-dataset)
                                 int-array))))
    ;;its trained now
    untrained))
...
(defrecord SmileClassification []
  ...
  (predict [system options trained-model-bytes coalesced-dataset]
    (let [trained-model (model/byte-array->model trained-model-bytes)
          label-map (ml-details/options->label-map options (::dataset/label-keys options))
