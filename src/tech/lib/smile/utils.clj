...
(defn prepend-data-constructor-arguments
  ...
  (let [data-constructor-arguments
        ...
        (if (contains? (:attributes entry-metadata) :attributes)
          (let [ecount-map (get-in options [::dataset/dataset-info ::dataset/key-ecount-map])
                attribute-array
                (->> (::dataset/feature-keys options)
                     (mapcat (fn [feature-key]
                               (let [nominal? (boolean
                                               (get-in options [:label-map
                                                                feature-key]))
                                     attribute (if nominal?
                                                 (NominalAttribute. (name feature-key))
                                                 (NumericAttribute. (name feature-key)))]
                                 (repeat (get ecount-map feature-key) attribute))))
...
(defn options->num-classes
  ^long [options]
  (long (if-let [retval
                 (get-in options [::dataset/dataset-info ::dataset/num-classes])]
          retval
          (throw (ex-info "Failed to find num classes"
                          {:dataset-info (::dataset/dataset-info options)})))))


(defn options->feature-ecount
  ^long [options]
  (long (if-let [retval
                 (get-in options [::dataset/dataset-info
                                  ::dataset/feature-ecount])]
          retval
          (throw (ex-info "Failed to find feature ecount"
                          {:dataset-info (::dataset/dataset-info options)
                           :options options})))))
